#!/bin/bash
# LFS 10.1 Build Script
# Builds the cross-toolchain and cross compiling temporary tools from chapters 5 and 6
# original 10.0 by Luís Mendes 14/09/2020 :)
# 
# updates to 10.1 by paul vanallsburg
...
../configure --prefix=$LFS/tools        \
	     ...
	     --disable-werror
make
make install -j1 ;rc=$?;echo $package_name $rc >> $LFS/sources/rc.log
finish
...
make
make install ;rc=$?;echo $package_name $rc >> $LFS/sources/rc.log
cd ..
...
make mrproper
make headers ;rc=$?;echo $package_name $rc >> $LFS/sources/rc.log
...
./configure --prefix=/usr                \
            ...
            --enable-widec
make
make DESTDIR=$LFS TIC_PATH=$(pwd)/build/progs/tic install ;rc=$?;echo $package_name $rc >> $LFS/sources/rc.log
