...
wget https://mesa.freedesktop.org/archive/mesa-21.2.2.tar.xz
wget https://www.linuxfromscratch.org/patches/blfs/svn/mesa-21.2.2-add_xdemos-1.patch
begin mesa-21.2.2 tar.xz
...
ninja
sudo ninja install
sudo install -v -dm755 /usr/share/doc/mesa-21.2.2 &&
sudo cp -rfv ../docs/* /usr/share/doc/mesa-21.2.2
finish
...
wget https://www.freedesktop.org/software/libevdev/libevdev-1.11.0.tar.xz
begin libevdev-1.11.0 tar.xz
./configure $XORG_CONFIG  &&
make
sudo make install
finish
...
wget https://www.freedesktop.org/software/libinput/libinput-1.19.0.tar.xz
begin libinput-1.19.0 tar.xz
mkdir build &&
cd    build &&
